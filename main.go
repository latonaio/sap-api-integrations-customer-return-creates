...
import (
	sap_api_caller "sap-api-integrations-customer-return-creates/SAP_API_Caller"
	sap_api_input_reader "sap-api-integrations-customer-return-creates/SAP_API_Input_Reader"
	"sap-api-integrations-customer-return-creates/config"
...
func main() {
	...
	inputSDC          := fr.ReadSDC("./Inputs/SDC_Customer_Return_Header_sample.json")
	accepter          := getAccepter(inputSDC)
	header            := inputSDC.ConvertToHeader()
	item              := inputSDC.ConvertToItem()

	caller.AsyncPostCustomerReturn(
		header,
		item,
		accepter,
	)
