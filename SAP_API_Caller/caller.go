...
import (
	...
	sap_api_output_formatter "sap-api-integrations-customer-return-creates/SAP_API_Output_Formatter"
	"strings"
	"sync"
...
func (c *SAPAPICaller) AsyncPostCustomerReturn(
...
func (c *SAPAPICaller) Header(header *requests.Header) {
	outputDataHeader, err := c.callCustomerReturnSrvAPIRequirementHeader("A_CustomerReturnHeader", header)
	if err != nil {
		c.log.Error(err)
		return
	}
...
func (c *SAPAPICaller) callCustomerReturnSrvAPIRequirementHeader(api string, header *requests.Header) (*sap_api_output_formatter.Header, error) {
...
func (c *SAPAPICaller) Item(item *requests.Item) {
	outputDataItem, err := c.callCustomerReturnSrvAPIRequirementItem("A_CustomerReturnHeader", item)
	if err != nil {
		c.log.Error(err)
		return
	}
...
func (c *SAPAPICaller) callCustomerReturnSrvAPIRequirementItem(api string, item *requests.Item) (*sap_api_output_formatter.Item, error) {
